...
while (length(accounts.left) > 0){
    ...
    error <- tryCatch(friends <- getFriends(screen_name=NULL, user_id=new.user,sleep=60,
                                            oauth_folder=oauth_folder, error=function(e) e))
    if (inherits(error,'error')) {
      cat("Error! On to the next one...left")
      next
    }
...
while (length(accounts.left) > 0){
    ...
    error <- tryCatch(friends <- getFriends(screen_name=NULL, user_id=new.user,sleep=60,
                                            oauth_folder=oauth_folder), error=function(e) e)
    if (inherits(error,'error')) {
      cat("Error! On to the next one...right")
      next
    }
...
library(twitteR)
load("~/twitter_crap/oauth_files/my_oauth") # loading OAuth Twitter token
