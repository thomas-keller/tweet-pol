...
#modified for pymongo 3.03 and so I can figure out what's going on.... (Thomas Keller)
#==============================================================================
...
def export_userlist(collection_name):
    #$gte is mongo slang for greater than in each field
    #so follows >=25, friends>=100, and tweets>=100
    ...
    for t in data:
        ...
        user_data[user_id] = "{0},{1},{2},{3},{4}".format(
            ...
            user_list[user_id]) 
    outhandle = open('/media/thomas/My Passport/postdoc/twitter_primary/temp/user_list_' + collection_name +'.csv', "w")
...
for col in collections:
        print col
        tweets[col] = {}
        d = open('/media/thomas/My Passport/postdoc/twitter_primary/temp/user_list_' + col + '.csv', 'r')
        d.readline()
        for line in d:
                try:
                        w = line.rstrip().split(",")
...
outhandle = open('/media/thomas/My Passport/postdoc/twitter_primary/temp/user_list_full.csv', 'w')
ulist = users.keys()
file_key = 'id_str,followers_count,lang,location,' + ",".join(collections)
