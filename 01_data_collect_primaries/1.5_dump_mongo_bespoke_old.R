...
dump_tweets<-function(files,ns,log,
...
for (f in files){
  ...
  ,error=function(err){
  print("something went wrong parsing that",err)
}, finally = { write.table(f,file='dem_processed_apr.txt',row.names=F,quote=F,append=T,sep=',',col.names=F) }
